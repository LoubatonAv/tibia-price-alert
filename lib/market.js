...
export async function getItemHistory(itemId, options = {}) {
  ...
  const res = await axios.get(`${API_URL}/item_history`, {
    params: {
      server: SERVER,
      item_id: id,
      start_days_ago: options.startDaysAgo ?? options.start_days_ago ?? 30,
      end_days_ago: options.endDaysAgo ?? options.end_days_ago ?? -1,
    },
  });
...
export async function getEvents(options = {}) {
  const res = await axios.get(`${API_URL}/events`, {
    params: {
      start_days_ago: options.startDaysAgo ?? options.start_days_ago ?? 240,
      end_days_ago: options.endDaysAgo ?? options.end_days_ago ?? -1,
    },
  });

  return Array.isArray(res.data) ? res.data : res.data?.events || res.data?.data || [];
}
