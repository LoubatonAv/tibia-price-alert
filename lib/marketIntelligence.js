  ...
  return rows
    .map((row) => ({
      ...
      buyOffer: safeNumber(
        getField(
          row,
          ["buy_offer", "buyOffer", "highest_buy", "highestBuy"],
          0,
        ),
      ),
      sellOffer: safeNumber(
        getField(
          row,
          ["sell_offer", "sellOffer", "lowest_sell", "lowestSell"],
          0,
        ),
      ),
      daySold: safeNumber(
        getField(row, ["day_sold", "daySold", "sold", "trades"], 0),
      ),
      dayAverageSell: safeNumber(
        getField(
          row,
          ["day_average_sell", "dayAverageSell", "average_sell", "avgSell"],
          0,
        ),
      ),
    }))
...
function analyzeApiHistory(history, item) {
  ...
  const sellPrices = recent
    .map((row) => row.sellOffer)
    .filter((price) => price > 0);
  const spreads = recent
    .map((row) =>
      row.buyOffer > 0 && row.sellOffer > 0
        ? ((row.sellOffer - row.buyOffer) / row.buyOffer) * 100
        : 0,
    )
  ...
  const priceChangePercent =
    firstSell > 0 ? ((lastSell - firstSell) / firstSell) * 100 : 0;
  ...
  const spreadStable =
    spreads.length >= 4 && maxSpread - minSpread <= 5 && avgSpread >= 4;
  const spreadLargeAndUnstable =
    spreads.length >= 4 && avgSpread >= 18 && maxSpread - minSpread >= 12;
  ...
  if (
    ...
    summaryParts.push(
      `7d ${priceChangePercent >= 0 ? "+" : ""}${priceChangePercent.toFixed(1)}%`,
    );
  }
...
function analyzeActivity(activity) {
  ...
  const worldRow = rows.find((row) => {
    const world = String(
      row.world || row.server || row.name || row.world_name || "",
    ).toLowerCase();
  ...
  const activeOffers = safeNumber(
    getField(
      worldRow,
      ["active_offers", "activeOffers", "offers", "offer_count", "offerCount"],
      0,
    ),
  );
  ...
  const offerToTradeRatio =
    trades > 0 ? activeOffers / trades : activeOffers > 0 ? 999 : 0;
  ...
  if (offerToTradeRatio >= 8) {
    ...
    warnings.push(
      "Many active offers compared with verified trades; market may be crowded.",
    );
  }
...
async function refreshEventsContext(state) {
  ...
  const lastChecked = previous?.lastCheckedAt
    ? new Date(previous.lastCheckedAt).getTime()
    : 0;
  const ageDays =
    lastChecked > 0
      ? (Date.now() - lastChecked) / 1000 / 60 / 60 / 24
      : Infinity;
  ...
  const eventNames = events
    .map((event) =>
      String(
        event.name || event.title || event.event || event.type || "",
      ).trim(),
    )
    .filter(Boolean)
    .slice(0, 10);
  ...
  const context = {
    ...
    summary: eventNames.length
      ? `Events context: ${eventNames.slice(0, 3).join(" / ")}`
      : "Events context: no recent event names parsed",
  };
...
export async function applyMarketIntelligence(items, options = {}) {
  ...
  const delayMs = Number(process.env.MARKET_INTELLIGENCE_DELAY_MS || 2500);
  ...
  for (const item of candidates) {
    ...
    if (ENABLE_ITEM_HISTORY) {
      try {
        const history = await getItemHistory(item.id, {
          startDaysAgo: ITEM_HISTORY_DAYS,
        });
    ...
    item.marketPressure = clamp(
      Number(item.marketPressure || 0) + pressureAdjust,
      0,
      100,
    );
    item.tradeabilityScore = clamp(
      Number(item.tradeabilityScore || 0) + tradeabilityAdjust,
      0,
      100,
    );
