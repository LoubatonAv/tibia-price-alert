...
import { getSmartDiscoveryItemIds } from "./smartDiscoveryItems.js";
...
export function getDiscoveryItemIds({ includeTracked = false } = {}) {
  const discoverySource = String(
    process.env.DISCOVERY_SOURCE || "hybrid",
  ).toLowerCase();
  ...
  const trackedIds = new Set(getTrackedItemIds());
  ...
  if (path) {
    ...
    manualIds = uniqueNumbers(ids).filter((id) => {
      if (blacklist.has(id)) return false;
      if (!includeTracked && trackedIds.has(id)) return false;
      return true;
    });
  }

  const smartIds = getSmartDiscoveryItemIds({
    includeTracked,
    trackedIds,
    blacklist,
  });

  if (discoverySource === "manual") {
    return manualIds;
  }

  if (discoverySource === "smart") {
    return smartIds;
  }

  // hybrid = manual first, then smart pool.
  return uniqueNumbers([...manualIds, ...smartIds]);
}
