...
function analyzeSpreadSustainability(history, item) {
  if (!history || history.length < 4) {
    return {
      ...
      sustainabilityAdvice: "Historical spread persistence data is still building.",
  ...
  const profitDownFor3 =
    ...
    profitPercents[profitPercents.length - 4] > profitPercents[profitPercents.length - 3] &&
    profitPercents[profitPercents.length - 3] > profitPercents[profitPercents.length - 2] &&
    profitPercents[profitPercents.length - 2] > profitPercents[profitPercents.length - 1];

  const spreadCompression = firstSpread > 0 ? ((firstSpread - lastSpread) / firstSpread) * 100 : 0;
  const stableSpread =
    spreads.length >= 4 &&
    Math.max(...spreads) - Math.min(...spreads) <= 4 &&
    lastSpread >= 5;

  const monthAverageSell = Number(item.monthAverageSell || item.month_average_sell || 0);
  const sellOffer = Number(item.sellOffer || item.sell_offer || 0);
  const volumeRatio = Number(item.volumeRatio || 0);
  const spikeRisk = monthAverageSell > 0 && sellOffer > monthAverageSell * 1.08 && volumeRatio < 0.9;

  if (spreadDownFor3 || profitDownFor3 || spreadCompression >= 35) {
    return {
      sustainabilityScore: -16,
      sustainabilityLabel: "SPREAD COLLAPSING",
      sustainabilityAdvice: "Spread has been shrinking across recent runs; sellers may already be eating the margin.",
  ...
  if (sellDownFor3 && volumeRatio < 0.8) {
    return {
      ...
      sustainabilityAdvice: "Sell price is falling while volume is not strong enough to absorb sellers.",
      collapsingSpread: true,
      stableSpread: false,
      spikeRisk,
    };
  ...
  if (spikeRisk) {
    return {
      ...
      sustainabilityAdvice: "Current price is above normal without strong volume confirmation.",
      collapsingSpread: false,
      stableSpread: false,
      spikeRisk: true,
    };
  ...
  if (stableSpread && volumeRatio >= 0.75) {
    return {
      ...
      sustainabilityAdvice: "Spread has stayed fairly stable across recent runs.",
      collapsingSpread: false,
      stableSpread: true,
      spikeRisk: false,
    };
  ...
  if (lastSpread >= prevSpread && volumeRatio >= 1) {
    return {
      ...
      sustainabilityAdvice: "Spread is not collapsing and today's demand is acceptable.",
      collapsingSpread: false,
      stableSpread: true,
      spikeRisk: false,
    };
  ...
  return {
    ...
    sustainabilityAdvice: "No clear proof yet that the spread will survive or collapse.",
    collapsingSpread: false,
    stableSpread: false,
    spikeRisk: false,
  };
...
export function calculateTradeabilityConviction(item) {
  ...
  const sellOffer = Number(item.sellOffer || item.sell_offer || 0);
  const dayAverageSell = Number(item.dayAverageSell || item.day_average_sell || 0);
  ...
  const expectedExitDays = daySold > 0 ? 1 : avgDailyVolume > 0 ? 1 / avgDailyVolume : 99;
  const stableEnough = Math.abs(dayVsMonthSell) <= 5;
  const stretchedVsDay = dayAverageSell > 0 && sellOffer > dayAverageSell * 1.05;
  const stretchedVsMonth = monthAverageSell > 0 && sellOffer > monthAverageSell * 1.08;
  ...
  if (sustainability.sustainabilityLabel === "SPREAD COLLAPSING") {
    ...
    warnings.push(sustainability.sustainabilityAdvice);
  } else if (["SPREAD HOLDING", "SPREAD STABLE"].includes(sustainability.sustainabilityLabel)) {
  ...
  if (daySold >= 45 && monthSold >= 1200 && volumeRatio >= 0.95) {
    ...
    notes.push("Exit looks clean: strong daily sales, deep monthly demand, and normal-or-better volume today.");
  } else if (daySold >= 20 && monthSold >= 500 && volumeRatio >= 0.7) {
    exitScore = 20;
    labels.push("GOOD EXIT");
    notes.push("Enough buyers exist for a realistic resale, but still use a patient listing.");
  } else if (daySold >= 8 && monthSold >= 220 && volumeRatio >= 0.45) {
    exitScore = 12;
    labels.push("MODERATE EXIT");
    notes.push("Exit is realistic, but not instant. Be ready to price competitively.");
    ...
    warnings.push("Very thin market. The visible spread may be mostly theoretical.");
  }
  score += exitScore;
  ...
  if (profit <= 0) {
  ...
  } else if (fakeSpreadRisk >= 75 || (rawSpreadPercent >= 45 && monthSold < 300)) {
    spreadQualityScore -= 32;
    labels.push("TRAP SPREAD");
    warnings.push("Spread is too large relative to liquidity. The exit price may not be real.");
    ...
    notes.push("Spread looks realistic: low fake-spread risk and enough margin.");
  ...
  if (marketPressure >= 70 || marketPressureLevel === "EXTREME") {
    ...
    warnings.push("Sell-side pressure is extreme. Avoid chasing the visible sell price.");
  ...
  if (item.fallingHard) {
    ...
    warnings.push("Price has been falling for several runs. Wait for stabilization.");
  ...
  if (dayVsMonthSell > 12 || stretchedVsMonth) {
    ...
    warnings.push("Current price is stretched above normal. You may be buying late.");
    ...
    notes.push("Current sell price is below monthly average while demand still exists.");
  }
  score += timingScore;
  ...
  if (possibleUndercutWar) {
    ...
    warnings.push("Sellers may be fighting lower. The exit can collapse quickly.");
  } else if (rawSpreadPercent > 25 && fakeSpreadRisk >= 35) {
    competitionScore -= 10;
    labels.push("UNDERCUT RISK");
    warnings.push("Large spread plus risk means other sellers can easily undercut you.");
  }
  ...
  if (brainScore < 75) {
    qualityPenalty -= 10;
    warnings.push("Brain Score is not strong enough for a top-tier conviction label.");
  }
  if (brainScore < 65) {
    qualityPenalty -= 10;
  }
  if (profit < 500) {
    qualityPenalty -= 8;
    warnings.push("Raw profit is small, so the trade may not justify attention.");
  }
  if (profit < 250) {
    qualityPenalty -= 7;
  }
  ...
  } else if (finalScore <= 38 || fakeSpreadRisk >= 70 || monthSold < 50 || profit <= 0) {
  ...
  if (conviction === "HIGH CONVICTION TRADE" && finalScore >= 74) {
    qualityTier = "ELITE";
  } else if (["HIGH CONVICTION TRADE", "MEDIUM CONVICTION TRADE"].includes(conviction) && finalScore >= 68) {
    qualityTier = "STRONG";
  ...
  const recommendation = (() => {
    ...
    return "Research only. Not clean enough for an automatic BUY.";
  })();
...
export function calculateScannerScore(item) {
  ...
  if (Number.isFinite(item.tradeabilityScore)) {
    const tradeabilityAdjustment = clamp((item.tradeabilityScore - 50) * 0.35, -18, 18);
    score += tradeabilityAdjustment;
    notes.push(`tradeability ${tradeabilityAdjustment >= 0 ? "+" : ""}${tradeabilityAdjustment.toFixed(1)}`);
  }
